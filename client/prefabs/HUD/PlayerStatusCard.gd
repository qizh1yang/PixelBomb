...
func refresh_from_player(player: Node) -> void:
	...
	if player.has_method("get_actual_explosion_ranges"):
		var ranges = player.get_actual_explosion_ranges()
		pow_up = ranges["up"]
		pow_down = ranges["down"]
		pow_left = ranges["left"]
		pow_right = ranges["right"]
