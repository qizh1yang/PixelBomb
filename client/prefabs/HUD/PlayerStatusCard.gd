...
func refresh_from_player(player: Node) -> void:
	...
	if bomb_label:
		bomb_label.text = "💣 %d/%d" % [cur_bombs, cap_bombs]
	...
	if power_label:
		power_label.text = "⚡ %d (↑%d ↓%d ←%d →%d)" % [cur_r, pow_up, pow_down, pow_left, pow_right]
		power_label.add_theme_color_override("font_color", Color("#ffb300"))
	...
	if shield_label:
		shield_label.text = "🛡️ %d/%d" % [shields, max_shields]
		if shields > 0:
