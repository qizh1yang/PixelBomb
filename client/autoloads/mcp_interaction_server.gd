...
func _ready() -> void:
	...
	var err: int = -1
	for i in range(10):
		err = _server.listen(listen_port, "127.0.0.1")
		if err == OK:
			break
		listen_port += 1
		
	if err != OK:
		push_error("McpInteractionServer: Failed to listen on port range starting from %d, error: %d" % [PORT, err])
		return
	print("McpInteractionServer: Listening on 127.0.0.1:%d" % listen_port)
